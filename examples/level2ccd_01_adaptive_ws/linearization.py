...
def set_IEA_UMaine(mdl, WindSpeeds=None):
...
class adaptive_ws_linearization():
    
    def __init__(self):
        self.mdl = FASTmodel()
        self._FAST_parameters = None
        self._Plant_parameters = None
        self._wind_speeds_set = []
        self._wind_speeds_current = None
        self._wind_speeds_sorted = []
        self._results_set = []
        self._results_current = []
        self._results_sorted = []
        
    def get_FAST_parameters(self):
        return self._FAST_parameters
    
    def set_FAST_parameters(self, FAST_parameters):
        self._FAST_parameters = FAST_parameters
        for k, v in FAST_parameters.items():
            setattr(self.mdl, k, v)
        self.mdl.set_GBRatio()
        
    FAST_parameters = property(get_FAST_parameters, set_FAST_parameters)
    
    def get_Plant_parameters(self):
        return self._Plant_parameters
    
    def set_Plant_parameters(self, plantdesign_list):
        self._Plant_parameters = plantdesign_list
        
    FAST_Plant_parameters = property(get_Plant_parameters, set_Plant_parameters)
        
    def set_initial_ws(self, wind_speeds):
        self._wind_speeds_current = np.array(wind_speeds)
        
    def calc_steady(self):
        # Setup wind speed
        WindSpeeds = self._wind_speeds_current
        if type(WindSpeeds) is np.ndarray:
            WindSpeeds = WindSpeeds.tolist()
        self.mdl.WindSpeeds = WindSpeeds
        # Setup trim case
        TrimCase = 3*np.ones(len(self.mdl.WindSpeeds), dtype=int)
        TrimCase[np.array(self.mdl.WindSpeeds) < self.mdl.v_rated] = 2
        self.mdl.TrimCase = TrimCase.tolist()
        # Prepare case input
        self.mdl.prepare_case_inputs(
            self.mdl.SOL_FLG_STEADY,
            self._Plant_parameters,
            self.mdl.fid,
        )
        # Run
        self.mdl.runFAST_steady()
    ...
    def calc_linear(self):
        ...
        # Prepare case input
        self.mdl.prepare_case_inputs(
            self.mdl.SOL_FLG_LINEAR,
            self._Plant_parameters,
            self.mdl.fid,
        )
        # Run
        self.mdl.runFAST_linear()
        
    def store_results(self):
        self._wind_speeds_set.append(self._wind_speeds_current)
        self._results_set.append(self._results_current)
        wind_speeds_all = np.array([])
        results_all = []
        for idx in range(0, len(self._wind_speeds_set)):
            wind_speeds_all = np.append(wind_speeds_all, self._wind_speeds_set[idx])
            results_all += self._results_set[idx]
        wind_speeds_sorted_indices = np.argsort(wind_speeds_all)
        wind_speeds_sorted = [wind_speeds_all[idx] for idx in wind_speeds_sorted_indices]
        results_sorted = [results_all[idx] for idx in wind_speeds_sorted_indices]
        self._wind_speeds_sorted = wind_speeds_sorted
        self._results_sorted = results_sorted
