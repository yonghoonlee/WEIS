...
class FASTmodel(LinearFAST):
    ...
    SOL_FLG_LINEAR = 1
    SOL_FLG_NONLINEAR_AT_LIN_COND = 2
    ...
    def prepare_case_inputs(self, SolutionStage, plantdesign_list, fid):
        ...
        if SolutionStage == self.SOL_FLG_NONLINEAR_AT_LIN_COND:
...
if __name__ == '__main__':
    ...
    #for pf in [0.5, 0.7, 0.8, 0.85, 0.9, 0.95, 0.975, 1.0, 1.025, 1.05, 1.1, 1.15, 1.2, 1.3, 1.5]:
        ...
        mdl = FASTmodel()
        mdl = set_IEA_UMaine(mdl, ws=np.linspace(7,11,3).tolist()) # mdl = set_IEA_UMaine(mdl) # WIND SPEED MODIFIED FOR DEBUG PURPOSE
        mdl.casename = 'pd'
        mdl.parallel = False
        mdl.debug_level = 2
